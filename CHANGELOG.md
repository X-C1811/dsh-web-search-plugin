...
- **自定义供应商**（`lib/custom.js` + `customProviders[]` + 设置段「自定义供应商」管理卡）：支持增删查改多个纯 REST 搜索后端。每条需 命名 + 凭据引用名 + 接口地址；API key 可稍后补（`auth: none` 无需 key）。删除当前生效项时 `provider` 自动回退到 `deepseek-official`。
