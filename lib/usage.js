...
import { resolveApiKey } from "./shared.js";
...
export function parseBraveRateLimitHeaders(headers) {
	...
	for (let i = 0; i < count; i++) {
		...
		windows.push({
			...
			used: Math.max(0, limit - Math.max(0, left)),
			windowSec,
...
export function parseTavilyUsageBody(body) {
	...
	const account = body.account !== null && typeof body.account === "object" ? body.account : {};
	const key = body.key !== null && typeof body.key === "object" ? body.key : {};
	const plan = typeof account.current_plan === "string" && account.current_plan.length > 0 ? account.current_plan : null;
	const limit = numOrNull(account.plan_limit) ?? numOrNull(key.limit);
	const remoteUsed = Math.max(num(account.plan_usage), num(key.usage));
	return {
		plan,
		limit,
		remoteUsed,
		keyUsed: numOrNull(key.usage),
		keyLimit: numOrNull(key.limit),
		paygoUsed: numOrNull(account.paygo_usage),
		paygoLimit: numOrNull(account.paygo_limit)
	};
}
...
export function mergeTavilyRemote(current, remote) {
	...
	const planChanged = remote.plan !== prev.plan || remote.limit !== prev.limit;
	const cycleReset = prev.fetchedAt > 0 && remote.remoteUsed < prev.remoteUsed;
