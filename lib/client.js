	...
	factory: (require) => {
		...
		var SearchCardForm = class {
			...
			providerOf() {
				...
				if (declared === "tavily" || declared === "brave" || declared === "deepseek-official") return declared;
		...
		function WebSearchCard(props) {
			...
			const provider = state.provider.text === "tavily" || state.provider.text === "brave" || state.provider.text === "deepseek-official" ? state.provider.text : (state.customProviders ?? []).some((entry) => entry?.id === state.provider.text) ? state.provider.text : "deepseek-official";
			const brave = provider === "brave";
			const deepseek = provider === "deepseek-official";
			const custom = !brave && !deepseek && (state.customProviders ?? []).some((entry) => entry?.id === provider);
			...
			return react.createElement("div", { className: "dshws_form" },
				...
				pair(
					...
					deepseek || brave || custom ? null : row("mode", t("mode"), null, selectInput({ id: "ws-mode", value: state.mode.text, disabled, onChange: (e) => actions.edit("mode", e.target.value), children: [
				...
				(!brave && !deepseek && !custom && keyed) ? pair(
				...
				(!brave && !deepseek && !custom) ? row("baseURL", t("baseURL"), t("baseURLHint"), textInput({ id: "ws-tavily-base", value: state.baseURL.text, disabled, onChange: (e) => actions.edit("baseURL", e.target.value) })) : null,
				deepseek ? pair(
