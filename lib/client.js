/**
 * Browser half of `web-search-tavily`: a plugin card under the
 * "settings → Plugins → Plugin configuration" surface that edits the
 * `web-search-tavily` settings namespace and its `TAVILY_API_KEY` credential.
 *
 ...
 *
 * @module web-search-tavily/client
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Staged form over the `web-search-tavily` settings namespace. Modeled on
		...
		function TavilyCard(props) {
			...
			return react.createElement("li", { style: { border: "1px solid var(--dsw-alias-border-l2)", background: "var(--dsw-alias-bg-layer-3)", borderRadius: "12px", listStyle: "none" } },
				...
				open ? react.createElement("div", { style: { borderTop: "1px solid var(--dsw-alias-border-l2)", margin: "0 16px", paddingBottom: "8px" } },
					...
					fieldRow(t("mode"), null, selectInput({ id: "plugin-config-tavily-mode", value: state.mode.text, disabled, onChange: (e) => props.edit("mode", e.target.value), children: [
					...
					state.mode.text === "keyed" ? fieldRow(t("apiKey"), t("apiKeyHint"), secretInput({ id: "plugin-config-tavily-key", value: state.apiKey.text, disabled: !state.apiKeyWritable, placeholder: state.apiKeyConfigured ? t("apiKeySet") : t("apiKeyUnset"), onChange: (e) => props.edit("apiKey", e.target.value) })) : null,
					state.mode.text === "keyed" ? fieldRow(t("apiKeyEnv"), t("apiKeyEnvHint"), textInput({ id: "plugin-config-tavily-keyref", value: state.apiKeyEnv.text, disabled, onChange: (e) => props.edit("apiKeyEnv", e.target.value), onReset: () => props.resetField("apiKeyEnv") })) : null,
					fieldRow(t("baseURL"), t("baseURLHint"), textInput({ id: "plugin-config-tavily-base", value: state.baseURL.text, disabled, onChange: (e) => props.edit("baseURL", e.target.value), onReset: () => props.resetField("baseURL") })),
					fieldRow(t("searchDepth"), null, selectInput({ id: "plugin-config-tavily-depth", value: state.searchDepth.text, disabled, onChange: (e) => props.edit("searchDepth", e.target.value), children: [
					...
					fieldRow(t("topic"), null, selectInput({ id: "plugin-config-tavily-topic", value: state.topic.text, disabled, onChange: (e) => props.edit("topic", e.target.value), children: [
					...
					fieldRow(t("includeAnswer"), t("includeAnswerHint"), checkboxInput({ id: "plugin-config-tavily-answer", checked: state.includeAnswer.text === "true", disabled, onChange: (e) => props.edit("includeAnswer", e.target.checked ? "true" : "") })),
					fieldRow(t("maxResults"), t("maxResultsHint"), textInput({ id: "plugin-config-tavily-max", numeric: true, value: state.maxResults.text, disabled, onChange: (e) => props.edit("maxResults", e.target.value), onReset: () => props.resetField("maxResults") })),
