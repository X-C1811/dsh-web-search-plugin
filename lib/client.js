...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const BUILTIN_REST_PROVIDERS = [
			{ id: "serper", name: "Serper", officialUrl: "https://serper.dev/", defaultBaseURL: "https://google.serper.dev", keyRequired: true, keyField: "serperApiKey", keyEnvField: "serperApiKeyEnv", baseURLField: "serperBaseURL" },
			{ id: "serpapi", name: "SerpApi", officialUrl: "https://serpapi.com/", defaultBaseURL: "https://serpapi.com", keyRequired: true, keyField: "serpapiApiKey", keyEnvField: "serpapiApiKeyEnv", baseURLField: "serpapiBaseURL" },
			{ id: "exa", name: "Exa", officialUrl: "https://dashboard.exa.ai/", defaultBaseURL: "https://api.exa.ai", keyRequired: true, keyField: "exaApiKey", keyEnvField: "exaApiKeyEnv", baseURLField: "exaBaseURL" },
			{ id: "searxng", name: "SearXNG", officialUrl: "https://docs.searxng.org/", defaultBaseURL: "https://searx.be", keyRequired: false, keyField: "", keyEnvField: "", baseURLField: "searxngBaseURL" },
			{ id: "scavio", name: "Scavio", officialUrl: "https://scavio.dev/", defaultBaseURL: "https://api.scavio.dev", keyRequired: true, keyField: "scavioApiKey", keyEnvField: "scavioApiKeyEnv", baseURLField: "scavioBaseURL" },
			{ id: "firecrawl", name: "Firecrawl", officialUrl: "https://firecrawl.dev/", defaultBaseURL: "https://api.firecrawl.dev", keyRequired: true, keyField: "firecrawlApiKey", keyEnvField: "firecrawlApiKeyEnv", baseURLField: "firecrawlBaseURL" }
		];
		...
		const en = {
			...
			apiKeyUnset: "No key set",
			apiKeyUnavailableHint: "Search is unavailable until a key is configured.",
			...
			proxy: "Proxy",
			proxyPlaceholder: "Leave blank to inherit the DSH proxy settings.",
			...
			getKey: "Get API key",
			ddgDisabled: "DuckDuckGo — use via SearXNG",
			ddgHint: "DuckDuckGo has no official search API. Pick SearXNG and point it at an instance that includes DuckDuckGo (most public ones do), or self-host one.",
			save: "Save",
			saving: "Saving…",
			discard: "Discard",
			unsaved: "Unsaved",
		...
		const zh = {
			...
			apiKeySet: "已配置密钥。",
			apiKeyUnset: "未配置密钥",
			apiKeyUnavailableHint: "未配置密钥前搜索不可用。",
			apiKeyEnv: "凭据引用名",
			...
			freshnessYear: "过去一年",
			proxy: "代理",
			proxyPlaceholder: "留空继承 DSH 全局代理设置",
			...
			collapse: "收起设置",
			getKey: "获取 API key",
			ddgDisabled: "DuckDuckGo — 请经 SearXNG 使用",
			ddgHint: "DuckDuckGo 没有官方搜索 API。请选择 SearXNG,并填入一个聚合了 DuckDuckGo 的实例地址(多数公共实例都有),或自建一个。",
			save: "保存",
			saving: "保存中…",
			discard: "放弃修改",
			unsaved: "未保存",
		...
		var SearchCardForm = class {
			constructor(scope, remote) {
				...
				this.specs = new Map([
					...
					textField("scavioApiKeyEnv"),
					textField("scavioBaseURL"),
					textField("firecrawlApiKeyEnv"),
					textField("firecrawlBaseURL")
			...
			restKeyRef(rest) {
				...
				return rest.id === "serper" ? "SERPER_API_KEY" : rest.id === "serpapi" ? "SERPAPI_API_KEY" : rest.id === "exa" ? "EXA_API_KEY" : rest.id === "scavio" ? "SCAVIO_API_KEY" : "FIRECRAWL_API_KEY";
			}
		...
		var SearchCardController = class {
			constructor(scope, remote) {
				this.scope = scope;
				this.remote = remote;
				this.form = new SearchCardForm(scope, remote);
			...
			projection() {
				return {
					...
					scavioApiKey: this.form.field("scavioApiKey"),
					scavioApiKeyEnv: this.form.field("scavioApiKeyEnv"),
					scavioBaseURL: this.form.field("scavioBaseURL"),
					firecrawlApiKey: this.form.field("firecrawlApiKey"),
					firecrawlApiKeyEnv: this.form.field("firecrawlApiKeyEnv"),
					firecrawlBaseURL: this.form.field("firecrawlBaseURL"),
		...
		function WebSearchCard(props) {
			...
			const provider = state.provider.text === "tavily" || state.provider.text === "brave" || state.provider.text === "deepseek-official" || builtinRest(state.provider.text) !== void 0 ? state.provider.text : "deepseek-official";
			...
			const keyHint = state.apiKeyConfigured ? t("apiKeyHint") : t("apiKeyHint") + " " + t("apiKeyUnavailableHint");
			...
			return react.createElement("div", { className: "dshws_form" },
				...
				pair(
					row("provider", t("provider"), null, selectInput({ id: "ws-provider", value: provider, disabled, onChange: (e) => actions.edit("provider", e.target.value), children: [
						...
						react.createElement("option", { value: "__ddg__", key: "ddg", disabled: true }, t("ddgDisabled"))
					] }),
					react.createElement("p", { className: "dshws_field_hint", style: { padding: "4px 0 0" } }, t("ddgHint"))),
				...
				deepseek ? pair(
					fieldRow(t("apiKey"), keyHint, secretInput({ id: "ws-deepseek-key", value: state.deepseekApiKey.text, disabled: !state.apiKeyWritable, placeholder: state.apiKeyConfigured ? t("apiKeySet") : t("apiKeyUnset"), onChange: (e) => actions.edit(DEEPSEEK_API_KEY_FIELD, e.target.value) })),
				...
				brave ? pair(
					fieldRow(t("apiKey"), keyHint, secretInput({ id: "ws-brave-key", value: state.braveApiKey.text, disabled: !state.apiKeyWritable, placeholder: state.apiKeyConfigured ? t("apiKeySet") : t("apiKeyUnset"), onChange: (e) => actions.edit(BRAVE_API_KEY_FIELD, e.target.value) })),
				...
				(tavily && keyed) ? pair(
					fieldRow(t("apiKey"), keyHint, secretInput({ id: "ws-tavily-key", value: state.apiKey.text, disabled: !state.apiKeyWritable, placeholder: state.apiKeyConfigured ? t("apiKeySet") : t("apiKeyUnset"), onChange: (e) => actions.edit(TAVILY_API_KEY_FIELD, e.target.value) })),
				...
				restMeta ? react.createElement("div", { className: "dshws_form" },
					restMeta.keyRequired && restMeta.officialUrl ? react.createElement("a", {
					...
					restMeta.keyRequired ? pair(
						fieldRow(t("apiKey"), keyHint, secretInput({ id: "ws-" + restMeta.id + "-key", value: state[restMeta.keyField]?.text ?? "", disabled: !state.apiKeyWritable, placeholder: state.apiKeyConfigured ? t("apiKeySet") : t("apiKeyUnset"), onChange: (e) => actions.edit(restMeta.keyField, e.target.value) })),
					...
					row(restMeta.baseURLField, t("baseURL"), t("baseURLHint"), textInput({ id: "ws-" + restMeta.id + "-base", value: state[restMeta.baseURLField]?.text || restMeta.defaultBaseURL || "", disabled, onChange: (e) => actions.edit(restMeta.baseURLField, e.target.value) })),
					maxResultsRow
				) : null,
		...
		function apply(ctx) {
			...
			const controller = new SearchCardController(ctx.settingsScope.bind({ namespace: NS }), ctx.remote);
