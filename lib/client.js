...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const fieldRow = (label, hint, children, meta) => react.createElement("div", { className: "dshws_field" },
			...
			children,
			meta?.invalid ? react.createElement("p", { className: "dshws_field_error" }, meta.t("invalidNumber")) : null,
			hint ? react.createElement("p", { className: "dshws_field_hint" }, hint) : null);
		...
		const textInput = (props) => react.createElement("input", { style: controlStyle, type: "text", ...props.numeric ? { inputMode: "numeric" } : {}, ...props });
		...
		const CSS_ID = "dsh-web-search-plugin/styles.css#2col";
		...
		if (typeof document !== "undefined") {
			...
			tag.textContent = [
				...
				".dshws_field_error{color:var(--dsw-alias-label-error);margin:0;font-size:12px;line-height:1.45}",
		...
		function WebSearchCard(props) {
			...
			const maxResultsRow = row("maxResults", t("maxResults"), t("maxResultsHint"), textInput({ id: "ws-max", numeric: true, value: state.maxResults.text, disabled, onChange: (e) => actions.edit("maxResults", e.target.value), className: state.maxResults.invalid ? "dshws_input_invalid" : void 0 }));
			return react.createElement("div", { className: "dshws_form" },
				...
				deepseek ? pair(
					row("maxTokens", t("maxTokens"), t("maxTokensHint"), textInput({ id: "ws-maxtokens", numeric: true, value: state.maxTokens.text, disabled, onChange: (e) => actions.edit("maxTokens", e.target.value), className: state.maxTokens.invalid ? "dshws_input_invalid" : void 0 })),
					row("maxUses", t("maxUses"), t("maxUsesHint"), textInput({ id: "ws-maxuses", numeric: true, value: state.maxUses.text, disabled, onChange: (e) => actions.edit("maxUses", e.target.value), className: state.maxUses.invalid ? "dshws_input_invalid" : void 0 }))
				) : null,
				(!brave && !deepseek) ? pair(
				...
				brave ? pair(
					row("country", t("country"), t("countryHint"), textInput({ id: "ws-country", value: state.country.text, disabled, onChange: (e) => actions.edit("country", e.target.value) })),
					row("searchLang", t("searchLang"), t("searchLangHint"), textInput({ id: "ws-lang", value: state.searchLang.text, disabled, onChange: (e) => actions.edit("searchLang", e.target.value) }))
				) : null,
				brave ? pair(
