...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Built-in REST providers (mirrors `lib/providers.js`; browser bundle is self-contained). */
		const BUILTIN_REST_PROVIDERS = [
			{ id: "serper", name: "Serper", officialUrl: "https://serper.dev/", keyRequired: true, keyField: "serperApiKey", keyEnvField: "serperApiKeyEnv", baseURLField: "serperBaseURL" },
			{ id: "serpapi", name: "SerpApi", officialUrl: "https://serpapi.com/", keyRequired: true, keyField: "serpapiApiKey", keyEnvField: "serpapiApiKeyEnv", baseURLField: "serpapiBaseURL" },
			{ id: "exa", name: "Exa", officialUrl: "https://dashboard.exa.ai/", keyRequired: true, keyField: "exaApiKey", keyEnvField: "exaApiKeyEnv", baseURLField: "exaBaseURL" },
			{ id: "searxng", name: "SearXNG", officialUrl: "https://docs.searxng.org/", keyRequired: false, keyField: "", keyEnvField: "", baseURLField: "searxngBaseURL" }
		];
		/** Find a built-in REST provider by id. */
		function builtinRest(id) {
			for (const entry of BUILTIN_REST_PROVIDERS) if (entry.id === id) return entry;
			return void 0;
		}
		/** Find a built-in REST provider by its literal key field name. */
		function builtinRestByKeyField(field) {
			for (const entry of BUILTIN_REST_PROVIDERS) if (entry.keyField === field) return entry;
			return void 0;
		}
		...
		/** Form fields carrying literal API keys (write-only, staged under the credentials domain). */
		const REST_KEY_FIELDS = BUILTIN_REST_PROVIDERS.map((entry) => entry.keyField).filter((field) => field !== "");
		const SECRET_FIELDS = new Set([TAVILY_API_KEY_FIELD, BRAVE_API_KEY_FIELD, DEEPSEEK_API_KEY_FIELD, ...REST_KEY_FIELDS]);
		...
		const en = {
			...
			customSectionHint: "Add your own search backends. Name, credential reference, and endpoint are required; the API key can be filled later (the entry stays unusable until a key is set).",
			customNew: "Add provider",
			customEmpty: "No custom providers yet.",
			customEdit: "Edit",
			customDelete: "Delete",
			customActive: "Active",
			customName: "Name",
			customApiKeyEnv: "Credential reference",
			customApiKeyEnvHint: "Name of the credential/env var resolved for this provider.",
			customBaseURL: "Endpoint",
			customBaseURLHint: "Base URL; /search is appended.",
			customApiKey: "API key",
			customApiKeyHint: "Optional now; the provider is unusable until a key is configured.",
			customSave: "Save provider",
			customCancel: "Cancel",
			customBlankRequired: "Name, credential reference, and endpoint are required.",
			customIdConflict: "That id collides with a built-in or existing provider.",
			customUnavailable: "Missing API key — unselectable until set.",
			customSelect: "Select this provider",
			customIsActive: "Selected engine",
			response: "Response shape",
			customAuthHeader: "Auth header",
			customAuth: "Auth",
			customNameHint: "A short label shown in the search-engine dropdown.",
			authBearer: "Bearer — key in the Authorization header (Tavily, Kimi, most AI APIs)",
			authHeader: "Custom header — key in a named header (e.g. Brave's X-Subscription-Token)",
			authNone: "None — no key needed (DuckDuckGo, keyless/Trial modes)",
			customAuthHeaderHint: "The exact header name the provider expects.",
			customAuthHint: "How the API key is sent. Bearer is the standard for most AI search APIs; choose Header only when the provider names a specific header (like Brave's X-Subscription-Token); choose None for keyless endpoints.",
			responseTavily: "Tavily-like — { results: [...] } + optional answer",
			responseBrave: "Brave-like — { web: { results: [...] } }",
			responseExa: "Exa-like — bare results array (semantic/neural search)",
			queryParam: "Query field name",
			queryParamHint: "The request-body field that carries the query. Tavily uses \"query\"; Serper uses \"q\". Leave blank to use the default \"query\".",
			customResponseHint: "These are loose shape hints, not exact schemas: parsing is lenient and auto-handles organic / web.results / results arrays and common field aliases (url/link, content/snippet/description/text). Pick the closest one — any of them usually parses a given endpoint; the choice mainly affects which answer block becomes the summary."
		};
		...
		const zh = {
			...
			customSectionHint: "添加你自己的搜索后端。命名、凭据引用名、接口地址必填；API key 可稍后补（未配 key 前该条不可用）。",
			customNew: "添加供应商",
			customEmpty: "还没有自定义供应商。",
			customEdit: "编辑",
			customDelete: "删除",
			customActive: "生效中",
			customName: "命名",
			customApiKeyEnv: "凭据引用名",
			customApiKeyEnvHint: "该供应商解析的凭据/环境变量名。",
			customBaseURL: "接口地址",
			customBaseURLHint: "基址；会自动拼接 /search。",
			customApiKey: "API Key",
			customApiKeyHint: "可选；未配置前该供应商不可用。",
			customSave: "保存供应商",
			customCancel: "取消",
			customBlankRequired: "命名、凭据引用名、接口地址为必填项。",
			customIdConflict: "该 id 与内置或已有供应商冲突。",
			customUnavailable: "缺少 API key——补齐前不可选。",
			customSelect: "选用此供应商",
			customIsActive: "当前搜索引擎",
			response: "响应形态",
			customAuthHeader: "鉴权请求头",
			customAuth: "鉴权方式",
			customNameHint: "显示在「搜索引擎」下拉框里的简短名称。",
			authBearer: "Bearer——把 key 放进 Authorization 请求头（Tavily、Kimi 及多数 AI 搜索接口）",
			authHeader: "自定义请求头——把 key 放进指定名称的请求头（如 Brave 的 X-Subscription-Token）",
			authNone: "无——无需 key（DuckDuckGo、keyless/试用模式）",
			customAuthHeaderHint: "服务商要求的请求头名。",
			customAuthHint: "你的 API key 怎么发送。Bearer 是多数 AI 搜索接口的标准；仅当你用的服务商指定了专门的请求头（如 Brave 的 X-Subscription-Token）才选「自定义请求头」；无鉴权的免费端点选「无」。",
			responseTavily: "Tavily 类——{ results: [...] } + 可选 answer 摘要",
			responseBrave: "Brave 类——{ web: { results: [...] } }",
			responseExa: "Exa 类——裸 results 数组（语义/神经搜索）",
			queryParam: "查询字段名",
			queryParamHint: "请求体里承载查询词的字段名。如 Tavily 用 \"query\"，Serper 用 \"q\"。留空使用默认值 \"query\"。",
			customResponseHint: "响应形态是宽松的「类」匹配，不是精确 schema：解析会自动兼容 organic / web.results / results 数组位置，字段名也能自动对齐（url/link、content/snippet/description/text 等）。选最接近的即可，一般都能解析；选项主要影响把哪个答案块作为摘要。"
		};
		...
		var SearchCardForm = class {
			constructor(scope, api) {
				...
				this.specs = new Map([
					...
					textField("serperApiKeyEnv"),
					textField("serperBaseURL"),
					textField("serpapiApiKeyEnv"),
					textField("serpapiBaseURL"),
					textField("exaApiKeyEnv"),
					textField("exaBaseURL"),
					textField("searxngBaseURL")
				...
				/** Custom-provider manager state: which entry is in the editor and its draft. */
				this.customManager = { editing: null, draft: null, error: null, saving: false };
			...
			actions() {
				return {
					...
					editCustom: (id) => {
						this.beginCustomEdit(id);
					},
					editCustomField: (field, value) => {
						this.editCustomField(field, value);
					},
					cancelCustom: () => {
						this.cancelCustomEditor();
					},
					saveCustom: () => {
						this.saveCustom();
					},
			...
			/** Open the editor for a brand-new custom provider. */
			beginCustomNew() {
				this.customManager = {
					editing: null,
					draft: { id: "custom-" + Math.random().toString(36).slice(2, 10), name: "", apiKeyEnv: "", baseURL: "", apiKey: "", auth: "bearer", authHeader: "", response: "tavily", queryParam: "" },
					error: null,
					saving: false
				};
				this.publish();
			}
			/** Open the editor pre-filled from an existing entry (edit by internal id). */
			beginCustomEdit(id) {
				const entry = this.customList().find((item) => item?.id === id);
				if (entry === void 0) return;
				this.customManager = {
					editing: id,
					draft: { id, name: entry.name ?? "", apiKeyEnv: entry.apiKeyEnv ?? "", baseURL: entry.baseURL ?? "", apiKey: "", auth: entry.auth || "bearer", authHeader: entry.authHeader ?? "", response: entry.response || "tavily", queryParam: entry.queryParam ?? "" },
					error: null,
					saving: false
				};
				this.publish();
			}
			/** Update one draft field while the editor is open. */
			editCustomField(field, value) {
				const manager = this.customManager;
				if (manager.draft === null) return;
				manager.draft = { ...manager.draft, [field]: value };
				manager.error = null;
				this.publish();
			}
			...
			/** Persist the editor draft (add or edit), then write its key to credentials. */
			async saveCustom() {
				const manager = this.customManager;
				if (manager.draft === null || manager.saving) return;
				const draft = manager.draft;
				const name = String(draft.name ?? "").trim();
				const apiKeyEnv = String(draft.apiKeyEnv ?? "").trim();
				const baseURL = String(draft.baseURL ?? "").trim();
				const auth = String(draft.auth || "bearer").trim();
				const authHeader = String(draft.authHeader ?? "").trim();
				const response = String(draft.response || "tavily").trim();
				const queryParam = String(draft.queryParam ?? "").trim();
				const needsKey = auth !== "none";
				if (name === "" || baseURL === "" || (needsKey && apiKeyEnv === "")) {
					manager.error = "blankRequired";
					this.publish();
					return;
				}
				const id = manager.editing ?? draft.id;
				const replacing = manager.editing !== null;
				// id uniqueness vs built-ins and other custom entries.
				if (!replacing && (id === "tavily" || id === "brave" || id === "deepseek-official" || this.customIds().has(id))) {
					manager.error = "idConflict";
					this.publish();
					return;
				}
				this.customManager = { ...manager, saving: true, error: null };
				this.publish();
				let landed = true;
				try {
					const list = this.customList().filter((entry) => !(replacing && entry?.id === id));
					list.push({ id, name, apiKeyEnv, baseURL, auth, authHeader, response, queryParam });
					landed = await this.store("customProviders", list) && landed;
				...
				} catch (_customSaveFailure) {
					landed = false;
				}
				this.customManager = { editing: null, draft: null, error: landed ? null : "saveFailed", saving: false };
				if (landed && replacing && this.providerOf() === id) this.readCredential();
				this.publish();
				this.showToast(landed ? "saved" : "failed");
			}
			/** Remove a custom provider; roll the active provider back to DeepSeek if deleted. */
			async deleteCustom(id) {
				await this.store("customProviders", this.customList().filter((entry) => entry?.id !== id));
				const stagedProvider = this.staged.get("provider");
				const stagedText = stagedProvider !== void 0 && !stagedProvider.clear ? stagedProvider.text.trim() : "";
				if (this.sectionValue("provider") === id || stagedText === id) {
					await this.store("provider", "deepseek-official");
					if (stagedText === id) this.staged.delete("provider");
				}
				this.readCredential();
				this.publish();
			}
			stage(field, edit) {
				...
				this.failed = false;
				this.publish();
				if (field === "provider" || field === "apiKeyEnv" || field === "braveApiKeyEnv" || field === "deepseekApiKeyEnv" || BUILTIN_REST_PROVIDERS.some((entry) => entry.keyEnvField === field)) this.readCredential();
			}
			snapshotOf() {
			...
			providerOf() {
				...
				if (declared === "tavily" || declared === "brave" || declared === "deepseek-official" || builtinRest(declared) !== void 0) return declared;
				if (this.customIds().has(declared)) return declared;
				return "deepseek-official";
			}
			/** Resolved `customProviders[]` from the live section (never null). */
			customList() {
				const value = this.sectionValue("customProviders");
				return Array.isArray(value) ? value : [];
			}
			/** Set of custom-provider internal ids. */
			customIds() {
				const ids = /* @__PURE__ */ new Set();
				for (const entry of this.customList()) if (entry != null && typeof entry.id === "string") ids.add(entry.id);
				return ids;
			}
			tavilyRef() {
			...
			refOf() {
				...
				if (provider === "brave") return this.braveRef();
				if (provider === "deepseek-official") return this.deepseekRef();
				const rest = builtinRest(provider);
				if (rest !== void 0 && rest.keyRequired) return this.restKeyRef(rest);
				for (const entry of this.customList()) {
					if (entry?.id === provider && typeof entry.apiKeyEnv === "string" && entry.apiKeyEnv.length > 0) return entry.apiKeyEnv;
				}
				return this.tavilyRef();
			}
			/** Credential reference for a built-in REST provider's key env field. */
			restKeyRef(rest) {
				const declared = this.sectionValue(rest.keyEnvField);
				if (declared !== void 0 && declared.length > 0) return declared;
				return rest.id === "serper" ? "SERPER_API_KEY" : rest.id === "serpapi" ? "SERPAPI_API_KEY" : "EXA_API_KEY";
			}
			...
			async writeKey(field, value) {
				let ref;
				if (field === BRAVE_API_KEY_FIELD) ref = this.braveRef();
				else if (field === DEEPSEEK_API_KEY_FIELD) ref = this.deepseekRef();
				else {
					const rest = builtinRestByKeyField(field);
					ref = rest !== void 0 ? this.restKeyRef(rest) : this.tavilyRef();
				}
				try {
		...
		var SearchCardController = class {
			...
			projection() {
				return {
					...
					serperApiKey: this.form.field("serperApiKey"),
					serperApiKeyEnv: this.form.field("serperApiKeyEnv"),
					serperBaseURL: this.form.field("serperBaseURL"),
					serpapiApiKey: this.form.field("serpapiApiKey"),
					serpapiApiKeyEnv: this.form.field("serpapiApiKeyEnv"),
					serpapiBaseURL: this.form.field("serpapiBaseURL"),
					exaApiKey: this.form.field("exaApiKey"),
					exaApiKeyEnv: this.form.field("exaApiKeyEnv"),
					exaBaseURL: this.form.field("exaBaseURL"),
					searxngBaseURL: this.form.field("searxngBaseURL"),
					apiKeyConfigured: this.form.credential.configured,
					apiKeyWritable: this.form.credential.writable,
					customProviders: this.form.customList(),
					customManager: this.form.customManager,
					activeIsCustom: this.form.customIds().has(this.form.providerOf()),
		...
		if (typeof document !== "undefined") {
			...
			tag.textContent = [
				...
				".dshws_btn{appearance:none;font:inherit;cursor:pointer;border-radius:6px;padding:8px 16px;font-size:12.5px;font-weight:500;line-height:1.5;display:inline-flex;align-items:center;justify-content:center;border:1px solid transparent;transition:filter .15s ease,opacity .15s ease}",
				".dshws_btn:disabled{cursor:not-allowed;opacity:.6}",
				".dshws_btn_primary{background:var(--dsw-alias-button-primary-fill,var(--dsw-alias-brand-primary,#3b82f6));color:var(--dsw-alias-label-primary-foreground,#fff);font-weight:600}",
				".dshws_btn_primary:hover:not(:disabled){filter:brightness(1.12)}",
				".dshws_btn_secondary{background:var(--dsw-alias-button-tool-bar-fill,var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.1)));color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l2,var(--dsw-alias-border-secondary,rgba(128,128,128,.25)))}",
				".dshws_btn_secondary:hover:not(:disabled){background:var(--dsw-alias-button-tool-bar-hover,var(--dsw-alias-interactive-bg-active,rgba(128,128,128,.16)))}",
				".dshws_btn_outline{background:transparent;border-color:var(--dsw-alias-border-l2,var(--dsw-alias-border-secondary,rgba(128,128,128,.25)));color:var(--dsw-alias-label-secondary)}",
				".dshws_btn_outline:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l1,rgba(128,128,128,.45));background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.1))}",
		...
		function WebSearchCard(props) {
			...
			const provider = state.provider.text === "tavily" || state.provider.text === "brave" || state.provider.text === "deepseek-official" || builtinRest(state.provider.text) !== void 0 ? state.provider.text : (state.customProviders ?? []).some((entry) => entry?.id === state.provider.text) ? state.provider.text : "deepseek-official";
			const brave = provider === "brave";
			const deepseek = provider === "deepseek-official";
			const tavily = provider === "tavily";
			const restMeta = builtinRest(provider);
			const custom = !brave && !deepseek && !tavily && restMeta === void 0 && (state.customProviders ?? []).some((entry) => entry?.id === provider);
			...
			return react.createElement("div", { className: "dshws_form" },
				...
				pair(
					row("provider", t("provider"), null, selectInput({ id: "ws-provider", value: provider, disabled, onChange: (e) => actions.edit("provider", e.target.value), children: [
						...
						...BUILTIN_REST_PROVIDERS.map((entry) => react.createElement("option", { value: entry.id, key: entry.id }, entry.name)),
						...(state.customProviders ?? []).map((entry) => react.createElement("option", { value: entry.id, key: entry.id }, entry.name || entry.id))
					] })),
					tavily ? row("mode", t("mode"), null, selectInput({ id: "ws-mode", value: state.mode.text, disabled, onChange: (e) => actions.edit("mode", e.target.value), children: [
				...
				custom ? react.createElement("div", { className: "dshws_quota" },
					react.createElement("p", { className: "dshws_quota_hint" }, t("customSectionHint"))) : null,
				...
				tavily ? row("baseURL", t("baseURL"), t("baseURLHint"), textInput({ id: "ws-tavily-base", value: state.baseURL.text, disabled, onChange: (e) => actions.edit("baseURL", e.target.value) })) : null,
				deepseek ? pair(
				...
				restMeta ? react.createElement("div", { className: "dshws_form" },
					restMeta.officialUrl ? react.createElement("a", {
						href: restMeta.officialUrl,
						target: "_blank",
						rel: "noopener noreferrer",
						className: "dshws_field_hint",
						style: { color: "var(--dsw-alias-brand-primary,#3b82f6)", textDecoration: "none", padding: "8px 0", display: "inline-block" }
					}, t("getKey") + " ↗") : null,
					restMeta.keyRequired ? pair(
						fieldRow(t("apiKey"), t("apiKeyHint"), secretInput({ id: "ws-" + restMeta.id + "-key", value: state[restMeta.keyField]?.text ?? "", disabled: !state.apiKeyWritable, placeholder: state.apiKeyConfigured ? t("apiKeySet") : t("apiKeyUnset"), onChange: (e) => actions.edit(restMeta.keyField, e.target.value) })),
						row(restMeta.keyEnvField, t("apiKeyEnv"), t("apiKeyEnvHint"), textInput({ id: "ws-" + restMeta.id + "-keyref", value: state[restMeta.keyEnvField]?.text ?? "", disabled, onChange: (e) => actions.edit(restMeta.keyEnvField, e.target.value) }))
					) : null,
					row(restMeta.baseURLField, t("baseURL"), t("baseURLHint"), textInput({ id: "ws-" + restMeta.id + "-base", value: state[restMeta.baseURLField]?.text ?? "", disabled, onChange: (e) => actions.edit(restMeta.baseURLField, e.target.value) })),
					maxResultsRow
				) : null,
				react.createElement("div", { style: { borderTop: "1px solid var(--dsw-alias-border-l2)", justifyContent: "flex-end", alignItems: "center", gap: "8px", padding: "12px 0 4px", display: "flex" } },
					react.createElement("button", {
						type: "button",
						className: "dshws_btn dshws_btn_secondary",
		...
		/** Custom-provider manager: list entries, add/edit/delete, and a small editor. */
		function CustomProvidersManager(props) {
			const { t, state, actions, disabled } = props;
			const list = state.customProviders ?? [];
			const editing = state.customManager?.editing != null;
			const editingNew = state.customManager?.editing === null && state.customManager?.draft != null;
			const draft = state.customManager?.draft;
			const active = state.provider.text;
			const editorOpen = editing || editingNew;
			const draftName = String(draft?.name ?? "").trim();
			const draftBase = String(draft?.baseURL ?? "").trim();
			const draftEnv = String(draft?.apiKeyEnv ?? "").trim();
			const needsKey = draft?.auth !== "none";
			const canSave = draft != null && draftName !== "" && draftBase !== "" && (needsKey ? draftEnv !== "" : true);
			const editorRow = (label, hint, children) => fieldRow(label, hint, children, {
				t,
				overridden: false,
				invalid: false,
				disabled,
			...
			return react.createElement("div", { className: "dshws_form", style: { marginTop: "4px" } },
				react.createElement("div", { className: "dshws_card_header" },
					react.createElement("div", { className: "dshws_card_headText" },
						react.createElement("span", { className: "dshws_card_name" }, t("customSection")),
						react.createElement("span", { className: "dshws_card_desc" }, t("customSectionHint"))),
					react.createElement("button", { type: "button", className: "dshws_btn dshws_btn_secondary", disabled: disabled || editorOpen, onClick: (e) => { e.preventDefault(); actions.newCustom(); } }, t("customNew"))),
				!editorOpen && list.length === 0 ? react.createElement("p", { className: "dshws_field_hint", style: { padding: "8px 0" } }, t("customEmpty")) : null,
				!editorOpen ? list.map((entry, index) => {
					const isActive = entry?.id === active;
					return react.createElement("div", { key: entry?.id ?? index, className: "dshws_quota", style: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" } },
						react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "2px", minWidth: 0 } },
							react.createElement("span", { className: "dshws_card_name", style: { fontSize: "13px" } }, entry?.name || entry?.id),
							react.createElement("span", { className: "dshws_field_hint", style: { fontSize: "11px" } }, (entry?.baseURL || "") + (isActive ? " · " + t("customActive") : ""))),
						react.createElement("div", { style: { display: "flex", gap: "8px", flex: "none", alignItems: "center" } },
							react.createElement("button", { type: "button", className: "dshws_btn dshws_btn_secondary", disabled, onClick: (e) => { e.preventDefault(); actions.editCustom(entry.id); } }, t("customEdit")),
							react.createElement("button", { type: "button", className: "dshws_btn dshws_btn_secondary", disabled, style: { color: "var(--dsw-alias-state-error-primary,#ef4444)" }, onClick: (e) => { e.preventDefault(); void actions.deleteCustom(entry.id); } }, t("customDelete"))));
				}) : null,
				editorOpen && draft != null ? react.createElement("div", { className: "dshws_form" },
					editorRow(t("customName"), t("customNameHint"), textInput({ id: "ws-custom-name", value: draft.name, disabled, onChange: (e) => actions.editCustomField("name", e.target.value) })),
					draft.auth !== "none" ? editorRow(t("customApiKeyEnv"), t("customApiKeyEnvHint"), textInput({ id: "ws-custom-env", value: draft.apiKeyEnv, disabled, onChange: (e) => actions.editCustomField("apiKeyEnv", e.target.value) })) : null,
					editorRow(t("customBaseURL"), t("customBaseURLHint"), textInput({ id: "ws-custom-base", value: draft.baseURL, disabled, onChange: (e) => actions.editCustomField("baseURL", e.target.value) })),
					editorRow(t("customApiKey"), t("customApiKeyHint"), secretInput({ id: "ws-custom-key", value: draft.apiKey, disabled, onChange: (e) => actions.editCustomField("apiKey", e.target.value) })),
					editorRow(t("customAuth"), t("customAuthHint"), selectInput({ id: "ws-custom-auth", value: draft.auth, disabled, onChange: (e) => actions.editCustomField("auth", e.target.value), children: [
						react.createElement("option", { value: "bearer", key: "bearer" }, t("authBearer")),
						react.createElement("option", { value: "header", key: "header" }, t("authHeader")),
						react.createElement("option", { value: "none", key: "none" }, t("authNone"))
					] })),
					draft.auth === "header" ? editorRow(t("customAuthHeader"), t("customAuthHeaderHint"), textInput({ id: "ws-custom-authheader", value: draft.authHeader, disabled, placeholder: "X-Subscription-Token", onChange: (e) => actions.editCustomField("authHeader", e.target.value) })) : null,
					editorRow(t("response"), t("customResponseHint"), selectInput({ id: "ws-custom-response", value: draft.response, disabled, onChange: (e) => actions.editCustomField("response", e.target.value), children: [
						react.createElement("option", { value: "tavily", key: "tavily" }, t("responseTavily")),
						react.createElement("option", { value: "brave", key: "brave" }, t("responseBrave")),
						react.createElement("option", { value: "exa", key: "exa" }, t("responseExa"))
					] })),
					editorRow(t("queryParam"), t("queryParamHint"), textInput({ id: "ws-custom-qparam", value: draft.queryParam, disabled, placeholder: "query", onChange: (e) => actions.editCustomField("queryParam", e.target.value) })),
					state.customManager?.error ? react.createElement("p", { className: "dshws_field_error" }, t(state.customManager.error === "idConflict" ? "customIdConflict" : state.customManager.error === "saveFailed" ? "saveFailed" : "customBlankRequired")) : !canSave ? react.createElement("p", { className: "dshws_field_error" }, t("customBlankRequired")) : null,
					react.createElement("div", { style: { display: "flex", gap: "8px", justifyContent: "flex-end", padding: "12px 0 4px" } },
						react.createElement("button", { type: "button", className: "dshws_btn dshws_btn_secondary", disabled: state.customManager?.saving, onClick: (e) => { e.preventDefault(); actions.cancelCustom(); } }, t("customCancel")),
						react.createElement("button", { type: "button", className: "dshws_btn dshws_btn_primary", disabled: !state.writable || state.customManager?.saving || !canSave, onClick: (e) => { e.preventDefault(); void actions.saveCustom(); } }, state.customManager?.saving ? t("saving") : t("customSave"))))
					: null);
		}
		...
		function makeSection(ctx, controller) {
			return function WebSearchSection() {
				...
				return react.createElement("div", { style: { padding: "16px" } },
					react.createElement("div", { style: { border: "1px solid var(--dsw-alias-border-l2)", background: "var(--dsw-alias-bg-layer-3)", borderRadius: "12px", padding: "4px 16px", marginBottom: "16px" } },
					...
					react.createElement("div", { style: { border: "1px solid var(--dsw-alias-border-l2)", background: "var(--dsw-alias-bg-layer-3)", borderRadius: "12px", padding: "4px 16px" } },
						react.createElement(CustomProvidersManager, { t: t, state: state, actions: controller.form.actions(), disabled: !state.writable })));
			};
		}
