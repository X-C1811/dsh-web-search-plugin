...
- **自定义供应商管理**：可自建多个纯 REST 搜索后端（增删查改）。每条需 命名 + 凭据引用名 + 接口地址；API key 可稍后补（`auth: none` 无需 key）。删除当前生效项时自动回退到 DeepSeek 官方。
- Tavily keyless 无需密钥即可用；Brave 需要订阅 token（若本机已有 `BRAVE_API_KEY` 凭据，可直接复用）。
- Tavily keyed / Brave 在设置卡展示额度进度条（DeepSeek 官方与 Tavily keyless / 自定义不展示）。
...
设置 → 网页搜索 → 自定义供应商，可**增删查改**多个纯 REST 搜索后端（每个 entry 为一个实例）：

| entry 键 | 默认值 | 含义 |
|---|---|---|
| `id` | 自动生成 | 内部稳定主键（如 `custom-a1b2c3`），生成后不变；不能与内置 `tavily` / `brave` / `deepseek-official` 冲突 |
| `name` | — | 显示名（搜索引擎下拉框里显示） |
| `apiKeyEnv` | — | 凭据引用名；`auth: none` 时可为空 |
| `baseURL` | — | 接口基址；请求为 `POST {baseURL}/search` |
| `auth` | `bearer` | 鉴权方式：`bearer`（`Authorization: Bearer <key>`）/ `header`（key 放自定义头）/ `none`（无鉴权） |
| `authHeader` | — | 仅 `auth: header` 使用：自定义鉴权头名（如 `X-Subscription-Token`） |
| `response` | `tavily` | 响应形态「类」提示（宽松匹配，非精确 schema）：`tavily`（`results[]`+`answer`）/ `brave`（`web.results[]`）/ `exa`（裸 `results[]`）。解析自动兼容 `organic` / `web.results` / `results` 数组位置与常见字段别名 |
| `queryParam` | `query` | 请求体里承载查询词的字段名；Serper 等接口用 `q` |
| `apiKey` | — | API key 字面量（走凭据域，不会回显） |

**协议范围**：自定义供应商仅覆盖**纯 REST 搜索 API** 一类（"一个查询 → 一组结果"）。"模型自带搜索工具"（如 DeepSeek 官方走 Anthropic Messages `web_search_20250305`、OpenAI 走 Responses API）属于 LLM 调用形态，不纳入自定义——内置 `deepseek-official` 已实现前者；MCP 亦不在本次范围。

**行为约定**：
- name + baseURL 必填；`auth` 非 `none` 时 apiKeyEnv 也必填；API key 可留空占位（该条可存在可改可删，但不可用，搜索报 `WEB_PROVIDER_CREDENTIAL_MISSING`）。`auth: none` 无需 key，立即可用。
- 删除**当前生效**的自定义供应商时，`provider` 自动回退到 `deepseek-official`。
- 所有配置（含自定义列表）落本机 `$DSH_HOME` profile settings；API key 只进本机凭据域，不上传、不跨 profile/机器同步。
